...
The deployed Gmail OAuth callback also completed for a controlled account on 14 September 2026: connection `CONNECTED`, requested read/send scopes granted, history checkpoint and watch expiry returned, and no provider error. This closes only the OAuth/watch-setup observation; bounded read/send and actual SES receipt remain open.
