...
@router.post("/projects/{project_id}/documents", response_model=UploadGrantRead, status_code=status.HTTP_201_CREATED, tags=["contracts"])
def post_contract_document(
    ...
    def create() -> UploadGrantRead:
        grant = create_upload_grant(session, context, project_id=project_id, object_key=request.object_key, mime_type=request.mime_type, expected_size_bytes=request.expected_size_bytes)
        return UploadGrantRead(document_id=grant.document.id, object_key=grant.document.object_key, object_version=grant.document.object_version, upload_token=grant.token, expires_at=grant.expires_at, expected_size_bytes=request.expected_size_bytes, expected_mime_type=request.mime_type, upload_url=grant.upload_url, upload_fields=grant.upload_fields)

    return execute_idempotent(session, context, f"POST /api/v1/projects/{project_id}/documents", idempotency_key, request, UploadGrantRead, create)


@router.get("/documents/{document_id}", response_model=DocumentRead, tags=["contracts"])
def contract_document(document_id: UUID, session: DbSession, context: CurrentContext) -> DocumentRead:
...
@router.post("/documents/{document_id}/complete-upload", response_model=DocumentRead, tags=["contracts"])
def complete_contract_upload(
    ...
    if request.content_base64 is not None:
        ...
        source = get_upload_source(session, context, document_id=document_id, upload_token=request.upload_token)
        session.rollback()
        try:
    ...
    def complete() -> DocumentRead:
        return DocumentRead.model_validate(complete_upload(session, context, document_id=document_id, upload_token=request.upload_token, content=content, declared_sha256=request.sha256, declared_mime_type=request.mime_type))

    return execute_idempotent(session, context, f"POST /api/v1/documents/{document_id}/complete-upload", idempotency_key, request, DocumentRead, complete)


@router.patch("/documents/{document_id}/candidate-scope", response_model=ScopeCandidateRead, tags=["contracts"])
def patch_candidate_scope(
    ...
    def correct() -> ScopeCandidateRead:
        candidate = correct_scope_candidate(session, context, candidate_id=request.candidate_id, corrected_text=request.corrected_text, reason=request.reason)
        if candidate.document_id != document_id:
            raise ValueError("Candidate does not belong to document")
        return ScopeCandidateRead(candidate_id=candidate.id, status=candidate.status, corrected_text=candidate.corrected_text)

    return execute_idempotent(session, context, f"PATCH /api/v1/documents/{document_id}/candidate-scope", idempotency_key, request, ScopeCandidateRead, correct)
...
def confirm_project_scope(
    ...
    def confirm() -> ScopeRead:
        version = confirm_scope(session, context, project_id=project_id, candidate_ids=request.candidate_ids)
        items = session.scalars(select(ScopeItem).join(ScopeVersionItem, ScopeVersionItem.scope_item_id == ScopeItem.id).where(ScopeVersionItem.scope_version_id == version.id, ScopeItem.tenant_id == context.tenant_id)).all()
        return ScopeRead(version=version.version, content_hash=version.content_hash, confirmed_at=version.confirmed_at, items=[ScopeItemRead.model_validate(item) for item in items])

    return execute_idempotent(session, context, f"POST /api/v1/projects/{project_id}/scope/confirm", idempotency_key, request, ScopeRead, confirm)
...
def _integration_read(session: DbSession, context: CurrentContext, connection_id: UUID) -> IntegrationConnectionRead:
    from services.integrations.gmail import read_gmail_health
    item = next((row for row in read_gmail_health(session, context) if row["id"] == connection_id), None)
    if item is None:
        raise ValidationError("Gmail connection was not found")
    return IntegrationConnectionRead.model_validate(item)

@router.post("/integrations/gmail/connect", response_model=GmailOAuthStartRead, tags=["integrations"])
...
@router.get("/integrations/gmail/callback", response_model=IntegrationConnectionRead, tags=["integrations"])
...
@router.post("/integrations/gmail/{connection_id}/reconnect", response_model=GmailOAuthStartRead, tags=["integrations"])
def reconnect_gmail_route(connection_id: UUID, session: DbSession, context: CurrentContext) -> GmailOAuthStartRead:
...
@router.get("/integrations/gmail", response_model=list[IntegrationConnectionRead], tags=["integrations"])
def gmail_connections_route(session: DbSession, context: CurrentContext) -> list[IntegrationConnectionRead]:
    from services.integrations.gmail import read_gmail_health
    return [IntegrationConnectionRead.model_validate(item) for item in read_gmail_health(session, context)]

@router.get("/integrations/health", response_model=list[IntegrationConnectionRead], tags=["integrations"])
def integrations_health_route(session: DbSession, context: CurrentContext) -> list[IntegrationConnectionRead]:
    return gmail_connections_route(session, context)

@router.delete("/integrations/gmail/{connection_id}", response_model=IntegrationConnectionRead, tags=["integrations"])
def disconnect_gmail_route(connection_id: UUID, session: DbSession, context: CurrentContext) -> IntegrationConnectionRead:
...
def gmail_push_webhook(body: dict[str, object], session: DbSession, x_goog_topic: Annotated[str | None, Header(alias="x-goog-topic")] = None) -> dict[str, str]:
    ...
    if isinstance(message, dict) and "data" in message:
        ...
        try:
            occurred_at = datetime.fromisoformat(published.replace("Z", "+00:00")) if published else datetime.now(UTC)
...
def route_integration_event(
    ...
    def route() -> RoutingResultRead:
        event = ingest_external_event(session, context, provider=request.provider, environment=request.environment, provider_event_id=request.provider_event_id, payload_hash=request.payload_hash, occurred_at=request.occurred_at)
        result = route_external_event(session, context, external_event_id=event.id, resource_type=request.resource_type, resource_id=request.resource_id, source_version=request.source_version, content_ref=request.content_ref, occurred_at=request.occurred_at, explicit_project_id=request.explicit_project_id, alias=request.alias, sender=request.sender, thread_id=request.thread_id)
        if isinstance(result, CommunicationEvent):
            return RoutingResultRead(kind="communication", id=result.id, status="MAPPED", project_id=result.project_id, resource_type=result.resource_type, resource_id=result.resource_id, source_version=result.source_version)
        if isinstance(result, RoutingDecision):
            return RoutingResultRead(kind="decision", id=result.id, status=result.status, candidate_projects=result.candidate_projects)
        raise ValueError("Unsupported routing result")

    return execute_idempotent(session, context, "POST /api/v1/integration-events/route", idempotency_key, request, RoutingResultRead, route)
...
def routing_decisions(session: DbSession, context: CurrentContext) -> list[RoutingDecisionRead]:
    ...
    return [RoutingDecisionRead.model_validate(item) for item in list_routing_decisions(session, context)]


@router.post("/requests", response_model=RequestRead, status_code=status.HTTP_201_CREATED, tags=["requests"])
def post_request(request: RequestCreate, session: DbSession, context: CurrentContext, idempotency_key: IdempotencyKey) -> RequestRead:
...
def clarify_request_route(request_id: UUID, request: RequestClarify, session: DbSession, context: CurrentContext, idempotency_key: IdempotencyKey) -> RequestRead:
    from services.contracts.requests import clarify_request

    def clarify() -> RequestRead:
        return RequestRead.model_validate(clarify_request(session, context, request_id=request_id, correction=request.correction))

    return execute_idempotent(session, context, f"POST /api/v1/requests/{request_id}/clarify", idempotency_key, request, RequestRead, clarify)


@router.post("/requests/{request_id}/merge", response_model=RequestRead, tags=["requests"])
def merge_request_route(request_id: UUID, request: RequestMerge, session: DbSession, context: CurrentContext, idempotency_key: IdempotencyKey) -> RequestRead:
    from services.contracts.requests import merge_requests

    def merge() -> RequestRead:
        return RequestRead.model_validate(merge_requests(session, context, target_request_id=request_id, source_request_id=request.source_request_id, reason=request.reason))

    return execute_idempotent(session, context, f"POST /api/v1/requests/{request_id}/merge", idempotency_key, request, RequestRead, merge)


@router.post("/integration-events/{event_id}/assign", response_model=RoutingResultRead, tags=["routing"])
def assign_integration_event(event_id: UUID, request: RoutingResolveRequest, session: DbSession, context: CurrentContext, idempotency_key: IdempotencyKey) -> RoutingResultRead:
    ...
    def assign() -> RoutingResultRead:
        result = resolve_routing_decision(session, context, decision_id=event_id, project_id=request.project_id, actor=context.subject)
        if isinstance(result, CommunicationEvent):
            return RoutingResultRead(kind="communication", id=result.id, status="MAPPED", project_id=result.project_id, resource_type=result.resource_type, resource_id=result.resource_id, source_version=result.source_version)
        return RoutingResultRead(kind="decision", id=result.id, status=result.status, candidate_projects=result.candidate_projects)

    return execute_idempotent(session, context, f"POST /api/v1/integration-events/{event_id}/assign", idempotency_key, request, RoutingResultRead, assign)


@router.post("/integration-events/{event_id}/replay", response_model=RoutingResultRead, tags=["routing"])
def replay_integration_event(event_id: UUID, request: RoutingResolveRequest, session: DbSession, context: CurrentContext, idempotency_key: IdempotencyKey) -> RoutingResultRead:
    from services.contracts.service import replay_external_event
    from services.domain.models import CommunicationEvent

    def replay() -> RoutingResultRead:
        result = replay_external_event(session, context, external_event_id=event_id, project_id=request.project_id)
        if isinstance(result, CommunicationEvent):
            return RoutingResultRead(kind="communication", id=result.id, status="MAPPED", project_id=result.project_id, resource_type=result.resource_type, resource_id=result.resource_id, source_version=result.source_version)
        return RoutingResultRead(kind="decision", id=result.id, status=result.status, candidate_projects=result.candidate_projects)

    return execute_idempotent(session, context, f"POST /api/v1/integration-events/{event_id}/replay", idempotency_key, request, RoutingResultRead, replay)

@router.get("/documents/{document_id}/download", response_model=DocumentDownloadRead, tags=["contracts"])
def download_contract_document(document_id: UUID, session: DbSession, context: CurrentContext) -> DocumentDownloadRead:
    from services.contracts.service import get_document
    ...
    try:
        ...
        download_url = None
    return DocumentDownloadRead(document_id=document.id, object_key=document.object_key, object_version=document.object_version, sha256=document.sha256, access_scope=f"tenant:{context.tenant_id}:project:{document.project_id}", download_url=download_url)

@router.get("/projects/{project_id}/documents", response_model=list[DocumentRead], tags=["contracts"])
def project_documents(project_id: UUID, session: DbSession, context: CurrentContext) -> list[DocumentRead]:
    from services.contracts.service import list_documents

    return [DocumentRead.model_validate(item) for item in list_documents(session, context, project_id)]


@router.get("/documents/{document_id}/candidates", response_model=list[DocumentCandidateRead], tags=["contracts"])
def document_candidates(document_id: UUID, session: DbSession, context: CurrentContext) -> list[DocumentCandidateRead]:
    from services.contracts.service import list_candidates

    return [DocumentCandidateRead.model_validate(item) for item in list_candidates(session, context, document_id)]


@router.post("/documents/{document_id}/extract-scope", response_model=StructureExtractionRead, tags=["contracts"])
...
def analysis_decisions(
    ...
    return [AnalysisDecisionRead.model_validate(item) for item in list_analysis_decisions(session, context, status=status, project_id=project_id, limit=limit)]


@router.get("/decisions/{decision_id}", response_model=AnalysisDecisionDetailRead, tags=["analysis"])
def analysis_decision(decision_id: UUID, session: DbSession, context: CurrentContext) -> AnalysisDecisionDetailRead:
    decision = get_analysis_decision(session, context, decision_id)
    revision = session.scalar(select(AnalysisDraftRevision).where(AnalysisDraftRevision.decision_id == decision.id, AnalysisDraftRevision.status == "CURRENT"))
    payload = AnalysisDecisionDetailRead.model_validate(decision)
    return payload.model_copy(update={"draft_revision_id": revision.id if revision else None, "draft_revision": revision.payload if revision else None})


@router.get("/decisions/{decision_id}/evidence", response_model=list[AnalysisEvidenceRead], tags=["analysis"])
def analysis_decision_evidence(decision_id: UUID, session: DbSession, context: CurrentContext) -> list[AnalysisEvidenceRead]:
    return [AnalysisEvidenceRead.model_validate(item) for item in get_decision_evidence(session, context, decision_id)]


@router.get("/workflows/{workflow_id}/trace", response_model=WorkflowTraceRead, tags=["analysis"])
def analysis_workflow_trace(workflow_id: UUID, session: DbSession, context: CurrentContext) -> WorkflowTraceRead:
...
def _change_order_read(session: DbSession, order: object) -> ChangeOrderRead:
    ...
    ) if order.current_revision_id else None
    return ChangeOrderRead.model_validate(order).model_copy(
        update={"revision": ProposalRevisionRead.model_validate(revision) if revision else None}
    )


@router.post("/decisions/{decision_id}/change-order", response_model=ChangeOrderRead, status_code=status.HTTP_201_CREATED, tags=["proposals"])
def assemble_change_order_route(
    ...
    def assemble() -> ChangeOrderRead:
        return _change_order_read(session, assemble_change_order(session, context, decision_id=decision_id))

    return execute_idempotent(session, context, f"POST /api/v1/decisions/{decision_id}/change-order", idempotency_key, {}, ChangeOrderRead, assemble)


@router.get("/change-orders/{change_order_id}", response_model=ChangeOrderRead, tags=["proposals"])
def get_change_order_route(change_order_id: UUID, session: DbSession, context: CurrentContext) -> ChangeOrderRead:
    from services.change_orders.service import read_change_order

    return _change_order_read(session, read_change_order(session, context, change_order_id=change_order_id))


@router.post("/change-orders/{change_order_id}/revisions", response_model=ChangeOrderRead, tags=["proposals"])
def create_change_order_revision_route(
    ...
    def create() -> ChangeOrderRead:
        create_revision(session, context, change_order_id=change_order_id, edits=request.model_dump(exclude_none=True))
        return _change_order_read(session, read_change_order(session, context, change_order_id=change_order_id))

    return execute_idempotent(session, context, f"/api/v1/change-orders/{change_order_id}/revisions", idempotency_key, request, ChangeOrderRead, create)


@router.post("/change-orders/{change_order_id}/approve", response_model=ChangeOrderRead, tags=["proposals"])
def approve_change_order_route(
    ...
    def approve() -> ChangeOrderRead:
        approve_freelancer(session, context, change_order_id=change_order_id, expected_row_version=request.expected_row_version, revision_id=request.revision_id, content_hash=request.content_hash)
        return _change_order_read(session, read_change_order(session, context, change_order_id=change_order_id))

    return execute_idempotent(session, context, f"/api/v1/change-orders/{change_order_id}/approve", idempotency_key, request, ChangeOrderRead, approve)


@router.post("/change-orders/{change_order_id}/reject", response_model=ChangeOrderRead, tags=["proposals"])
def reject_change_order_route(change_order_id: UUID, request: FreelancerDecisionRequest, session: DbSession, context: CurrentContext, idempotency_key: IdempotencyKey) -> ChangeOrderRead:
    from services.change_orders.service import freelancer_decide, read_change_order
    def reject() -> ChangeOrderRead:
        freelancer_decide(session, context, change_order_id=change_order_id, decision="REJECTED", comment=request.comment)
        return _change_order_read(session, read_change_order(session, context, change_order_id=change_order_id))
    return execute_idempotent(session, context, f"/api/v1/change-orders/{change_order_id}/reject", idempotency_key, request, ChangeOrderRead, reject)

@router.post("/change-orders/{change_order_id}/waive", response_model=ChangeOrderRead, tags=["proposals"])
def waive_change_order_route(change_order_id: UUID, request: FreelancerDecisionRequest, session: DbSession, context: CurrentContext, idempotency_key: IdempotencyKey) -> ChangeOrderRead:
    from services.change_orders.service import freelancer_decide, read_change_order
    def waive() -> ChangeOrderRead:
        freelancer_decide(session, context, change_order_id=change_order_id, decision="WAIVED", comment=request.comment)
        return _change_order_read(session, read_change_order(session, context, change_order_id=change_order_id))
    return execute_idempotent(session, context, f"/api/v1/change-orders/{change_order_id}/waive", idempotency_key, request, ChangeOrderRead, waive)

@router.post("/change-orders/{change_order_id}/withdraw", response_model=ChangeOrderRead, tags=["proposals"])
def withdraw_change_order_route(change_order_id: UUID, request: WithdrawalRequest, session: DbSession, context: CurrentContext, idempotency_key: IdempotencyKey) -> ChangeOrderRead:
    from services.change_orders.service import read_change_order, withdraw_change_order
    def withdraw() -> ChangeOrderRead:
        withdraw_change_order(session, context, change_order_id=change_order_id, reason=request.reason)
        return _change_order_read(session, read_change_order(session, context, change_order_id=change_order_id))
...
def exchange_capability_route(request: CapabilityExchangeRequest, response: Response, session: DbSession) -> dict[str, str]:
...
@public_router.get("/change-orders/{change_order_id}", response_model=ClientReviewRead, tags=["client"])
...
@public_router.get("/change-orders/{change_order_id}/receipt", response_model=ClientReceiptRead, tags=["client"])
...
def refresh_client_session_route(response: Response, session: DbSession, scopeguard_client_session: str | None = Cookie(default=None)) -> dict[str, str]:
...
def client_request_changes_route(change_order_id: UUID, request: ClientDecisionRequest, response: Response, session: DbSession, csrf_token: Annotated[str | None, Header(alias="X-CSRF-Token")] = None, scopeguard_client_session: str | None = Cookie(default=None)) -> dict[str, str]:
    ...
    order = client_decide(session, session_token=scopeguard_client_session, csrf_token=csrf_token, change_order_id=change_order_id, expected_row_version=request.expected_row_version, revision_id=request.revision_id, content_hash=request.content_hash, decision="REQUEST_CHANGES", comment=request.comment)
    _public_response(response)
    return {"change_order_id": str(order.id), "revision_id": str(request.revision_id), "status": order.status}

@public_router.post("/change-orders/{change_order_id}/reject", response_model=dict[str, str], tags=["client"])
def client_reject_route(change_order_id: UUID, request: ClientDecisionRequest, response: Response, session: DbSession, csrf_token: Annotated[str | None, Header(alias="X-CSRF-Token")] = None, scopeguard_client_session: str | None = Cookie(default=None)) -> dict[str, str]:
    from services.change_orders.service import client_decide
    if not scopeguard_client_session or not csrf_token:
        raise ValidationError("Client session and CSRF token are required")
    order = client_decide(session, session_token=scopeguard_client_session, csrf_token=csrf_token, change_order_id=change_order_id, expected_row_version=request.expected_row_version, revision_id=request.revision_id, content_hash=request.content_hash, decision="REJECTED", comment=request.comment)
    _public_response(response)
    return {"change_order_id": str(order.id), "revision_id": str(request.revision_id), "status": order.status}


@public_router.post("/change-orders/{change_order_id}/approve", response_model=ClientReceiptRead, tags=["client"])
