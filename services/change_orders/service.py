...
def _unprotect(value: str) -> str:
    ...
    try:
        ...
        return AESGCM(hashlib.sha256(key_material.encode("utf-8")).digest()).decrypt(envelope[:12], envelope[12:], b"scopeguard:client-capability").decode("utf-8")
    except (ValueError, TypeError, UnicodeDecodeError) as exc:
        raise ValidationError("Capability envelope is invalid") from exc

def _queue_freelancer_notification(session: Session, context: TrustedContext, order: ChangeOrder, revision: ProposalRevision) -> None:
    recipient = (context.email or "").strip().casefold()
    if not recipient:
        raise ValidationError("A verified freelancer notification address is required")
    payload: dict[str, object] = {"change_order_id": str(order.id), "revision_id": str(revision.id), "recipient": recipient, "subject": f"Approval required: {revision.title}", "token_ciphertext": revision.token_ciphertext, "approval_path": "/c"}
    action = prepare_external_action(session, tenant_id=order.tenant_id, project_id=order.project_id, action_key=f"change-order-notification:{order.id}:{revision.id}", provider="ses", operation="send_change_order", approved_payload=payload)
    notification = session.scalar(select(Notification).where(Notification.tenant_id == order.tenant_id, Notification.change_order_id == order.id, Notification.revision_id == revision.id, Notification.channel == "SES").with_for_update())
    if notification is None:
        notification = Notification(tenant_id=order.tenant_id, change_order_id=order.id, revision_id=revision.id, channel="SES", verified_recipient=recipient, action_id=action.id)
        session.add(notification)
    else:
        notification.action_id = action.id
    enqueue_job(session, tenant_id=order.tenant_id, project_id=order.project_id, kind="notification.send", payload_ref={"action_id": str(action.id), "notification_id": str(notification.id)}, correlation_id=context.correlation_id)
...
def _proposal_payload(
    ...
    *,
    ...
    required = ("title", "requested_change", "deliverables", "client_explanation", "subject", "plain_text_body", "html_body", "recipient_email", "evidence_reference_ids")
...
def _new_revision(
    ...
    *,
    ...
    revision = ProposalRevision(
        ...
        canonical_artifact_ref={"store": "proposal-artifacts", "format": "canonical-json", "schema_version": 1},
...
def assemble_change_order(
    ...
    *,
    ...
    if order is None:
        ...
        session.add(order)
        session.flush()
    _new_revision(session, order=order, project=project, request=request, contact=contact, draft=payload)
...
def create_revision(
    ...
    *,
    ...
    request = session.scalar(select(RequestRecord).where(RequestRecord.id == order.request_id, RequestRecord.tenant_id == context.tenant_id))
    ...
    revision, _ = _new_revision(session, order=order, project=project, request=request, contact=contact, draft=payload)
    return revision
...
def approve_freelancer(
    ...
    *,
    ...
    project = _project(session, context, session.scalar(select(ChangeOrder.project_id).where(ChangeOrder.id == change_order_id, ChangeOrder.tenant_id == context.tenant_id)) or UUID(int=0))
    order = session.scalar(
        select(ChangeOrder).where(ChangeOrder.id == change_order_id, ChangeOrder.tenant_id == context.tenant_id).with_for_update()
    )
    if order is None:
        raise NotFoundError("Change order was not found")
    project = _project(session, context, order.project_id)
    if order.row_version != expected_row_version:
        raise ConflictError("Change order changed; refresh before approving", details={"current_row_version": order.row_version})
    revision = _current_revision(session, order)
    ...
    request = session.scalar(select(RequestRecord).where(RequestRecord.id == order.request_id, RequestRecord.tenant_id == context.tenant_id).with_for_update())
    ...
    for reference_id in revision.evidence_reference_ids:
        try:
            ...
            continue
    evidence = session.scalars(select(EvidenceReference).where(EvidenceReference.id.in_(evidence_ids), EvidenceReference.tenant_id == context.tenant_id, EvidenceReference.project_id == project.id)).all() if evidence_ids else []
    bundle_ids = {item.bundle_id for item in evidence}
    stale_bundle = session.scalar(select(EvidenceBundle).where(EvidenceBundle.id.in_(bundle_ids), EvidenceBundle.stale_at.is_not(None), EvidenceBundle.stale_at <= _now())) if bundle_ids else None
    ...
    cap = session.scalar(select(ClientCapability).where(ClientCapability.revision_id == revision.id, ClientCapability.tenant_id == context.tenant_id, ClientCapability.revoked_at.is_(None)).with_for_update())
    if cap is None:
    ...
    enqueue_job(session, tenant_id=context.tenant_id, project_id=order.project_id, kind="change_order.send", payload_ref={"action_id": str(action.id), "change_order_id": str(order.id), "revision_id": str(revision.id)}, correlation_id=context.correlation_id)
    session.flush()
    return order, action


def _session_scope(session: Session, session_token: str, csrf_token: str | None = None) -> tuple[ClientSession, ClientCapability, ProposalRevision, ChangeOrder]:
    row = session.scalar(select(ClientSession).where(ClientSession.session_hash == _hash(session_token)).with_for_update())
    now = _now()
...
def exchange_capability(session: Session, *, token: str) -> CapabilityCredentials:
    ...
    capability = session.scalar(select(ClientCapability).where(ClientCapability.token_hash == _hash(token)).with_for_update())
    now = _now()
    if capability is None or capability.purpose != "REVIEW" or capability.activated_at is None or capability.revoked_at is not None or capability.expires_at is None or _utc(capability.expires_at) <= now:
        raise ValidationError("Invalid client capability")
    if capability.consumed_at is not None:
        order = session.get(ChangeOrder, capability.change_order_id)
        if order is None or order.status != "CLIENT_APPROVED":
            raise ValidationError("Invalid client capability")
        receipt = session.scalar(select(ClientCapability).where(ClientCapability.change_order_id == order.id, ClientCapability.revision_id == capability.revision_id, ClientCapability.client_id == capability.client_id, ClientCapability.purpose == "RECEIPT", ClientCapability.revoked_at.is_(None)).order_by(ClientCapability.created_at.desc()))
        if receipt is None or receipt.expires_at is None or _utc(receipt.expires_at) <= now:
            raise ValidationError("Invalid client capability")
        session_token = secrets.token_urlsafe(32)
        csrf_token = secrets.token_urlsafe(32)
        client_session = ClientSession(capability_id=receipt.id, session_hash=_hash(session_token), csrf_hash=_hash(csrf_token), purpose="RECEIPT", expires_at=now + timedelta(hours=24))
        session.add(client_session)
        session.flush()
        return CapabilityCredentials(receipt, "", client_session, session_token, csrf_token)
    session_token = secrets.token_urlsafe(32)
    csrf_token = secrets.token_urlsafe(32)
    client_session = ClientSession(capability_id=capability.id, session_hash=_hash(session_token), csrf_hash=_hash(csrf_token), purpose="REVIEW", expires_at=now + timedelta(hours=24))
    session.add(client_session)
    session.flush()
...
def read_client_review(session: Session, *, session_token: str) -> dict[str, Any]:
    ...
    if _unused_capability.purpose != "REVIEW" or _unused_capability.consumed_at is not None or order.status not in {"AWAITING_CLIENT_APPROVAL", "SEND_PENDING"}:
...
def accept_client(
    ...
    *,
    ...
    if order.id != change_order_id or capability.revision_id != revision_id or revision.canonical_artifact_hash != content_hash:
    ...
    project = session.scalar(select(Project).where(Project.id == order.project_id, Project.tenant_id == capability.tenant_id).with_for_update())
    locked_order = session.scalar(select(ChangeOrder).where(ChangeOrder.id == order.id, ChangeOrder.tenant_id == capability.tenant_id).with_for_update())
    ...
    if capability.consumed_at is not None:
        prior = session.scalar(select(Approval).where(Approval.revision_id == revision.id, Approval.actor_type == "CLIENT", Approval.provenance["idempotency_key"].as_string() == client_idempotency_key))
        existing = session.scalar(select(PaymentRequest).where(PaymentRequest.accepted_revision_id == revision.id))
        if prior is not None and existing is not None:
            return AcceptanceResult(order, existing, _receipt_credentials_for_existing(session, capability, revision, now=_now()))
        raise ConflictError("Client capability has already been consumed")
    if order.row_version != expected_row_version:
        raise ConflictError("Client offer changed; reload before accepting")
    action = session.scalar(select(ExternalAction).where(ExternalAction.action_key == f"change-order-send:{order.id}:{revision.id}", ExternalAction.tenant_id == capability.tenant_id).with_for_update())
    eligible = order.status == "AWAITING_CLIENT_APPROVAL" or (order.status == "SEND_PENDING" and action is not None and action.state in {"DISPATCHING", "UNKNOWN_OUTCOME"})
    if not eligible:
    ...
    if existing is not None:
        return AcceptanceResult(order, existing, _receipt_credentials(session, capability, revision, now=_now()))
    approval = Approval(tenant_id=capability.tenant_id, project_id=order.project_id, change_order_id=order.id, revision_id=revision.id, actor_type="CLIENT", actor_identifier=f"client:{capability.client_id}", content_hash=content_hash, decision="APPROVED", provenance={"model": "bearer-capability", "idempotency_key": client_idempotency_key})
    session.add(approval)
    now = _now()
    ...
    next_version = (session.scalar(select(func.max(ScopeVersion.version)).where(ScopeVersion.project_id == project.id, ScopeVersion.tenant_id == capability.tenant_id)) or previous_scope.version) + 1
    resulting = ScopeVersion(tenant_id=capability.tenant_id, project_id=project.id, version=next_version, parent_version_id=previous_scope.id, source_revision_id=revision.id, confirmation_actor=f"client:{capability.client_id}", content_hash=canonical_sha256({"parent": previous_scope.content_hash, "revision": revision.canonical_artifact_hash}),)
    session.add(resulting)
    session.flush()
    links = session.scalars(select(ScopeVersionItem).where(ScopeVersionItem.scope_version_id == previous_scope.id)).all()
    for link in links:
        session.add(ScopeVersionItem(tenant_id=capability.tenant_id, project_id=project.id, scope_version_id=resulting.id, scope_item_id=link.scope_item_id))
    project.current_scope_version_id = resulting.id
    amendment = ScopeAmendment(tenant_id=capability.tenant_id, project_id=project.id, accepted_revision_id=revision.id, previous_scope_version_id=previous_scope.id, resulting_scope_version_id=resulting.id, operations_json=[{"operation": "ADD_CHANGE_ORDER", "change_order_id": str(order.id), "revision_id": str(revision.id)}])
    session.add(amendment)
    payment = PaymentRequest(tenant_id=capability.tenant_id, project_id=project.id, accepted_revision_id=revision.id, total_minor=revision.total_minor, tax_minor=revision.tax_minor, currency=revision.currency, status="CREATION_PENDING")
    session.add(payment)
...
def _receipt_credentials(session: Session, source: ClientCapability, revision: ProposalRevision, *, now: datetime) -> CapabilityCredentials:
    token = secrets.token_urlsafe(32)
    cap = ClientCapability(tenant_id=source.tenant_id, project_id=source.project_id, change_order_id=source.change_order_id, revision_id=revision.id, client_id=source.client_id, token_hash=_hash(token), content_hash=revision.canonical_artifact_hash, purpose="RECEIPT", activated_at=now, expires_at=now + timedelta(days=30))
    session.add(cap)
    session.flush()
    session_token = secrets.token_urlsafe(32)
    csrf = secrets.token_urlsafe(32)
    client_session = ClientSession(capability_id=cap.id, session_hash=_hash(session_token), csrf_hash=_hash(csrf), purpose="RECEIPT", expires_at=now + timedelta(hours=24))
    session.add(client_session)
    session.flush()
...
def begin_gmail_send(session: Session, *, action_id: UUID) -> dict[str, Any]:
    ...
    action = session.scalar(select(ExternalAction).where(ExternalAction.id == action_id).with_for_update())
    ...
    order = session.scalar(select(ChangeOrder).where(ChangeOrder.id == order_id, ChangeOrder.tenant_id == action.tenant_id).with_for_update())
...
def record_gmail_send_success(
    ...
    *,
    ...
    if action.state == "RECEIPT_CONFIRMED":
        attempt = session.scalar(select(ActionAttempt).where(ActionAttempt.action_id == action.id).order_by(ActionAttempt.attempt_number.desc()).limit(1))
        if attempt is not None:
            attempt.provider_request_id = provider_message_id
            attempt.outcome_ref = {"provider_message_id": provider_message_id, "late_callback": True}
        action.receipt_ref = {**(action.receipt_ref or {}), "provider_message_id": provider_message_id}
        session.flush()
        return action
    ...
    if action.state == "UNKNOWN_OUTCOME":
        ...
        complete_action(session, action_id=action.id, provider_request_id=provider_message_id, outcome_ref={"provider_message_id": provider_message_id})
    order = session.scalar(select(ChangeOrder).where(ChangeOrder.id == UUID(str(expected["change_order_id"])), ChangeOrder.tenant_id == action.tenant_id).with_for_update())
...
def _audit_change_order(session: Session, order: ChangeOrder, *, actor: str, action: str, before: str, after: str, correlation_id: UUID, metadata: dict[str, object] | None = None) -> None:
    session.add(AuditEvent(tenant_id=order.tenant_id, project_id=order.project_id, actor=actor, action=action, resource_type="change_order", resource_id=str(order.id), resource_revision=order.row_version, correlation_id=correlation_id, before_state={"status": before}, after_state={"status": after}, content_digest=canonical_sha256({"change_order_id": str(order.id), "before": before, "after": after, "row_version": order.row_version}), safe_metadata=metadata or {}))

def _revoke_capabilities(session: Session, order_id: UUID, *, now: datetime) -> None:
    session.query(ClientCapability).filter(ClientCapability.change_order_id == order_id, ClientCapability.revoked_at.is_(None)).update({"revoked_at": now}, synchronize_session=False)

def _cancel_pending_send(session: Session, order: ChangeOrder) -> None:
    from services.workers.durable import cancel_action

    action = session.scalar(select(ExternalAction).where(ExternalAction.action_key.like(f"change-order-send:{order.id}:%"), ExternalAction.tenant_id == order.tenant_id).order_by(ExternalAction.created_at.desc()).with_for_update())
    if action is not None and action.state in {"READY", "RETRY_WAIT"}:
        cancel_action(session, action_id=action.id)

def freelancer_decide(session: Session, context: TrustedContext, *, change_order_id: UUID, decision: str, comment: str | None = None) -> ChangeOrder:
    if decision not in {"REJECTED", "WAIVED"}:
        raise ValidationError("Unsupported freelancer decision")
    order = session.scalar(select(ChangeOrder).where(ChangeOrder.id == change_order_id, ChangeOrder.tenant_id == context.tenant_id).with_for_update())
    if order is None:
        raise NotFoundError("Change order was not found")
    if order.status != "AWAITING_FREELANCER_APPROVAL":
    ...
    session.add(Approval(tenant_id=order.tenant_id, project_id=order.project_id, change_order_id=order.id, revision_id=revision.id, actor_type="FREELANCER", actor_identifier=context.subject, content_hash=revision.canonical_artifact_hash, decision=decision, comment=comment, provenance={"model": "authenticated-owner"}))
    before = order.status
    order.status = target
    order.row_version += 1
    ...
    _audit_change_order(session, order, actor=context.subject, action=f"freelancer_{decision.lower()}", before=before, after=target, correlation_id=context.correlation_id, metadata={"comment_present": bool(comment)})
    session.flush()
    return order

def withdraw_change_order(session: Session, context: TrustedContext, *, change_order_id: UUID, reason: str | None = None) -> ChangeOrder:
    order = session.scalar(select(ChangeOrder).where(ChangeOrder.id == change_order_id, ChangeOrder.tenant_id == context.tenant_id).with_for_update())
    if order is None:
        raise NotFoundError("Change order was not found")
    if order.status in {"CLIENT_APPROVED", "WITHDRAWN", "EXPIRED", "REJECTED_BY_CLIENT", "REJECTED_BY_FREELANCER"}:
    ...
    _audit_change_order(session, order, actor=context.subject, action="withdraw", before=before, after=order.status, correlation_id=context.correlation_id, metadata={"reason_present": bool(reason)})
    session.flush()
    return order

def client_decide(session: Session, *, session_token: str, csrf_token: str, change_order_id: UUID, expected_row_version: int, revision_id: UUID, content_hash: str, decision: str, comment: str | None = None) -> ChangeOrder:
    if decision not in {"REQUEST_CHANGES", "REJECTED"}:
        raise ValidationError("Unsupported client decision")
    _session_row, capability, revision, order = _session_scope(session, session_token, csrf_token)
    if capability.purpose != "REVIEW" or capability.consumed_at is not None or order.id != change_order_id or revision.id != revision_id or revision.canonical_artifact_hash != content_hash:
    ...
    session.add(Approval(tenant_id=capability.tenant_id, project_id=order.project_id, change_order_id=order.id, revision_id=revision.id, actor_type="CLIENT", actor_identifier=f"client:{capability.client_id}", content_hash=content_hash, decision=decision, comment=comment, provenance={"model": "bearer-capability"}))
    before = order.status
    order.status = target
    order.row_version += 1
    request = session.get(RequestRecord, order.request_id, with_for_update=True)
    if request is not None:
        ...
        if decision == "REQUEST_CHANGES":
            enqueue_job(session, tenant_id=capability.tenant_id, project_id=project.id, kind="change_order.freelancer_review", payload_ref={"change_order_id": str(order.id), "revision_id": str(revision.id)}, correlation_id=uuid4())
    _revoke_capabilities(session, order.id, now=now)
    _audit_change_order(session, order, actor=f"client:{capability.client_id}", action="client_request_changes" if decision == "REQUEST_CHANGES" else "client_reject", before=before, after=target, correlation_id=uuid4(), metadata={"comment_present": bool(comment)})
    session.flush()
    return order
...
def refresh_client_session(session: Session, *, session_token: str) -> CapabilityCredentials:
    ...
    csrf = secrets.token_urlsafe(32)
    new_row = ClientSession(capability_id=capability.id, session_hash=_hash(new_session_token), csrf_hash=_hash(csrf), purpose=row.purpose, expires_at=_now() + timedelta(hours=24))
    session.add(new_row)
    session.flush()
...
def expire_stale_offers(session: Session, *, now: datetime | None = None) -> int:
    current = now or _now()
    count = 0
    orders = session.scalars(select(ChangeOrder).where(ChangeOrder.status.in_(("SEND_PENDING", "AWAITING_CLIENT_APPROVAL")))).all()
    for order in orders:
        cap = session.scalar(select(ClientCapability).where(ClientCapability.change_order_id == order.id, ClientCapability.purpose == "REVIEW", ClientCapability.revoked_at.is_(None)).order_by(ClientCapability.created_at.desc()).with_for_update())
        ...
        _cancel_pending_send(session, order)
        _audit_change_order(session, order, actor="system", action="offer_expired", before=before, after=order.status, correlation_id=uuid4())
        count += 1
    pending = session.scalars(select(ChangeOrder).where(ChangeOrder.status == "SEND_PENDING")).all()
    for order in pending:
        action = session.scalar(select(ExternalAction).where(ExternalAction.action_key.like(f"change-order-send:{order.id}:%"), ExternalAction.tenant_id == order.tenant_id).order_by(ExternalAction.created_at.desc()))
        if action is None or action.state not in {"READY", "RETRY_WAIT"}:
            continue
        approval = session.scalar(select(Approval).where(Approval.change_order_id == order.id, Approval.actor_type == "FREELANCER", Approval.decision == "APPROVED").order_by(Approval.created_at.desc()))
        if approval is not None and _utc(approval.created_at) + timedelta(hours=24) <= current:
            ...
            _audit_change_order(session, order, actor="system", action="unsent_offer_expired", before=before, after=order.status, correlation_id=uuid4())
            count += 1
    session.flush()
    return count
...
def record_ses_notification_success(session: Session, *, action_id: UUID, provider_message_id: str) -> Notification:
    from services.workers.durable import complete_action
    action = session.scalar(select(ExternalAction).where(ExternalAction.id == action_id).with_for_update())
    ...
    notification = session.scalar(select(Notification).where(Notification.action_id == action.id).with_for_update())
    if notification is None:
...
def record_ses_notification_unknown(session: Session, *, action_id: UUID, reason: str) -> Notification:
    from services.workers.durable import mark_action_unknown
    action = session.scalar(select(ExternalAction).where(ExternalAction.id == action_id).with_for_update())
    if action is None or action.provider != "ses":
        raise NotFoundError("SES notification action was not found")
...
def reconcile_gmail_unknown(session: Session, *, action_id: UUID, matches: list[dict[str, str]]) -> ExternalAction:
    action = session.scalar(select(ExternalAction).where(ExternalAction.id == action_id).with_for_update())
    if action is None or action.provider != "gmail" or action.operation != "send":
        raise NotFoundError("Gmail send action was not found")
    if action.state != "UNKNOWN_OUTCOME":
    ...
    expected = action.approved_payload
    exact = [item for item in matches if item.get("recipient") == expected.get("recipient") and item.get("plain_text_body") == expected.get("plain_text_body") and item.get("content_hash") == expected.get("content_hash") and item.get("marker") == expected.get("marker") and item.get("provider_message_id")]
    if len(exact) == 1:
        return record_gmail_send_success(session, action_id=action.id, provider_message_id=exact[0]["provider_message_id"], recipient=str(expected["recipient"]), plain_text_body=str(expected["plain_text_body"]), content_hash=str(expected["content_hash"]))
    if len(exact) > 1:
