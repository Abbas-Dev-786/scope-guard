...
def test_evidence_gate_overturns_change_and_blocks_conflict() -> None:
    ...
    preliminary = ScopeRoleOutput(
        ...
        reason="New",
        uncertainty="LOW",
        matched_amendment_ids=[uuid.uuid4()],
    )
    covered = EvidenceRoleOutput(
        final_classification="PREVIOUSLY_APPROVED",
        supporting_references=[
            EvidenceReferenceOutput(
                reference_id=reference, relation="SUPPORTS", claim="Accepted amendment"
            )
        ],
    ...
    assert (
        apply_evidence_gate(
            preliminary, covered, accepted_amendment_ids=set(preliminary.matched_amendment_ids)
        ).route
        == "COVERED"
    )
    conflict = covered.model_copy(
        update={
            "contradictory_references": [
                EvidenceReferenceOutput(
                    reference_id=uuid.uuid4(), relation="CONTRADICTS", claim="Conflicting promise"
                )
            ]
        }
    )
...
def test_roles_are_read_only_and_manifest_fails_closed() -> None:
    ...
    graph = RoleGraph()
    stages = graph.run_pipeline(
        {},
        {
            name: lambda _payload, _limits: {"ok": True}
            for name in ("scope", "evidence", "impact", "change_order", "communication")
        },
    )
...
def test_dataset_has_frozen_split_and_three_passing_runs() -> None:
    ...
    assert (
        len(
            {item.project_id for item in development}.intersection(
                item.project_id for item in held_out
            )
        )
        == 0
    )
    ...
    def fixture_classifier(case: EvaluationInput) -> EvaluationPrediction:
        text = case.text.casefold()
        if not case.evidence_complete or case.contradictory:
            ...
            phrase in text
            for phrase in ("amendment", "accepted change order", "previously approved")
        ):
            classification = "PREVIOUSLY_APPROVED"
        elif any(
            phrase in text
            for phrase in (
                "no implementation",
            ...
            classification = "NOT_A_SCOPE_REQUEST"
        else:
            classification = "IN_SCOPE"
        return EvaluationPrediction(classification, case.allowed_reference_ids)

    reports = run_three_held_out_evaluations(fixture_classifier)
    assert all(report.passed for report in reports)
    assert all(report.count == 60 and report.reference_validity == 1.0 for report in reports)


def test_evaluation_rejects_invented_references_and_hides_gold_label() -> None:
    _, held_out = split_evaluation_dataset()
    assert not hasattr(held_out[0].model_input(), "label")
    metrics = evaluate_predictions(
        held_out,
        lambda _case: EvaluationPrediction("POTENTIAL_SCOPE_CHANGE", ("invented",)),
    )
    assert metrics.invalid_references == len(held_out)
...
def terms_factory(tmp_path: Path) -> tuple[TrustedContext, sessionmaker[Session], uuid.UUID]:
    engine = create_engine(
        f"sqlite+pysqlite:///{tmp_path / 'terms.db'}", connect_args={"check_same_thread": False}
    )
    ...
    with factory.begin() as session:
        session.add(
            User(
                id=TENANT,
                cognito_sub="phase4-analysis",
                verified_email="analysis@example.com",
...
def test_frozen_terms_use_database_versions_and_expected_fixture_price(
    terms_factory: tuple[TrustedContext, sessionmaker[Session], uuid.UUID],
) -> None:
        ...
        tasks=[
            {
                ...
                "high_hours": "18",
                "assumptions": ["No verified history is available"],
                "dependencies": ["Client metadata"],
            }
        ],
        low_hours="10",
        recommended_hours="14",
...
def test_prepare_analysis_persists_safe_proposal_and_trace(tmp_path: Path) -> None:
    engine = create_engine(
        f"sqlite+pysqlite:///{tmp_path / 'workflow.db'}", connect_args={"check_same_thread": False}
    )
    Base.metadata.create_all(engine)
    factory = sessionmaker(bind=engine, expire_on_commit=False)
    with factory.begin() as session:
        session.add(
            User(
                id=TENANT,
                cognito_sub="workflow-owner",
                verified_email="workflow@example.com",
                timezone="Asia/Kolkata",
            )
        )
        pref = PreferenceVersion(
        ...
        bundle = EvidenceBundle(
            ...
            searched_sources=[],
            cutoff=__import__("datetime").datetime.now(__import__("datetime").UTC),
            completeness="COMPLETE",
        )
        session.add(bundle)
        session.flush()
        reference = EvidenceReference(
            ...
            source_id="doc",
            source_version="v1",
            exact_excerpt_ref="chunk:1",
            content_hash="b" * 64,
            locator={},
            fetched_at=bundle.cutoff,
        ...
        preliminary = ScopeRoleOutput(
            classification="POTENTIAL_SCOPE_CHANGE",
            request_summary="Add OIDC",
            reason="New integration",
            uncertainty="MEDIUM",
        )
        evidence = EvidenceRoleOutput(
            final_classification="POTENTIAL_SCOPE_CHANGE",
            supporting_references=[
                EvidenceReferenceOutput(
                    reference_id=reference.id, relation="SUPPORTS", claim="OIDC is new"
                )
            ],
            completeness="COMPLETE",
            temporal_coverage="current",
        ...
        result = prepare_analysis(
            ...
            project_id=project.id,
            request_id=request.id,
            input_snapshot={"summary": request.summary},
            preliminary=preliminary,
            evidence=evidence,
            impact=impact,
            change_order=ChangeOrderRoleOutput(
                title="OIDC integration",
                requested_change="Add OIDC",
                deliverables=["OIDC login"],
                client_explanation="This is additional work.",
            ),
            communication=CommunicationRoleOutput(
                subject="OIDC change",
                plain_text_body="Please approve.",
                html_body="<p>Please approve.</p>",
...
def test_missing_or_stale_model_price_pauses_work() -> None:
    ...
    fresh = ReviewedModelPrices(
        version="prices-v1", reviewed_at=datetime.now(UTC), prices={"fixture": ModelPrice(2, 4)}
    )
    ...
    stale = ReviewedModelPrices(
        version="prices-v1",
        reviewed_at=datetime.now(UTC) - timedelta(days=31),
        prices={"fixture": ModelPrice(2, 4)},
    )
    with pytest.raises(ConflictError):
